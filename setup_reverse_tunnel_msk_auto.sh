...
"
echo "Host $HOSTNAME.tunnel
  Port $PORT_NUMBER
  HostName $MIDDLEMAN_SERVER
  User $MIDDLEMAN_USERNAME" >> ~/.ssh/config_tunnel

apt-get -y install sshpass 
cat ~/.ssh/config_tunnel  | sshpass -p 'aas1@3' ssh -o StrictHostKeyChecking=no $MIDDLEMAN_USERNAME@$MIDDLEMAN_SERVER 'cat >> ~/.ssh/config'
